...
const fs = require("fs");
let filename = "movies-collection-4";
...
const normalizeMovieData = (data, movieId, nowDate) => {
  ...
  for(let movie of tmdbMoviesData) {
    ...
    try{
      ...
      if(response.status == 200) {
        ...
        let { translationsData, ActorRoleData, MovieActorData, ActorCrewData, imageData, videoData, keywordData } = normalizeMovieData(data, data.id, nowDate);
